...
function updateOutput(){
    ...
    savedArr.slice(pageOffset*maxLoaded, (pageOffset + 1)*maxLoaded).forEach(map => {
        if (map) {
            ...
            var routeShorten = map["Route"].split(" -> ").join("_").split(" ").join("-");
            strOut += `<button class="buy-button ${map["Company"]} ${map["Date"]} ${routeShorten}">Buy</button>`
            strOut += "</p>";
        }
    });
    ...
    for(var i = 0; i < buyButtons.length; i++){
        const buyButtonClasses = Array.from(buyButtons[i].classList).slice(1);
        buyButtons[i].onclick = () => {
            API.APICall(buyButtonClasses);
        }
    }
